...
function ksPPF(x:number,y:number){
    const ks_table = [  [0  ,0.20       ,0.10       ,0.05       ,0.02       ,0.01       ,0.005      ,0.002      ,0.001  ],
                        ...
                        [51	,1.07	    ,1.22	    ,1.36	    ,1.52	    ,1.63	    ,1.73	    ,1.85       ,1.95]]
    return (x <= 50) ? ks_table[x][ks_table[0].indexOf(y)] : (ks_table[51][ks_table[0].indexOf(y)])/Math.sqrt(x);
}
